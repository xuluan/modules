    ...
    std::vector<std::vector<double>>& out_data)
{
    ...
    if (data_width < 1) {
        throw std::runtime_error("get_diverge_data() failed: invalid 'in_data' parameter, width is 0."); 
    }

    size_t data_height = in_data[0].size();

    if (data_height < 1) {
        throw std::runtime_error("get_diverge_data() failed: invalid 'in_data' parameter, height is 0");
    }
